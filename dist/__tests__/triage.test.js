...
import { deriveAction, findSimilarPRs, buildDraftComment, isCacheStale, } from "../triage.js";
...
describe("findSimilarPRs", () => {
    function makeEntry(number, embedding) {
        return {
            number,
            title: `PR #${number}`,
            body: "",
    ...
    it("limits to top 5 results", () => {
        const target = [1, 0];
        const entries = Array.from({ length: 10 }, (_, i) => makeEntry(i + 2, [1, 0.01 * i]));
        const results = findSimilarPRs(target, 1, entries, 0);
