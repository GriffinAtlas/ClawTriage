...
import { loadEnrichmentCache, enrichPRs, } from "./enrichment.js";
...
export async function batchTriage(owner, repo, options) {
    ...
    const entries = batchPRs.map((pr) => {
        ...
        const recommendedAction = deriveBatchAction(clusterIdx !== null, quality.score, visionAlignment);
        return {
            prNumber: pr.number,
            title: pr.title,
            user: pr.user,
...
function deriveBatchAction(isDuplicate, qualityScore, visionAlignment) {
...
function computeStats(entries, clusters) {
