...
import { loadIssueEnrichmentCache, enrichIssues, } from "./issue-enrichment.js";
...
export async function issueBatchTriage(owner, repo, options) {
    ...
    const batchIssues = allIssues.map((issue) => ({
        number: issue.number,
        title: issue.title,
        body: issue.body,
        user: issue.user,
    ...
    if (needsEmbedding.length > 0) {
        ...
        const texts = needsEmbedding.map((issue) => sanitize(`${issue.title} ${issue.body.slice(0, 500)}`));
        try {
            ...
            console.error(`[Issue Batch] Embedding failed, continuing without new embeddings:`, err.message);
        }
    }
    ...
    const issueClusterIndex = new Map();
    ...
    if (!skipVision) {
        ...
        if (visionDoc) {
            try {
                ...
                console.error(`[Issue Batch] Vision alignment failed, degrading gracefully:`, err.message);
                for (const issue of batchIssues) {
                    if (!visionResults.has(issue.number)) {
                        visionResults.set(issue.number, { alignment: "error", reason: `Vision batch failed: ${err.message}` });
                    }
                }
            }
        }
    ...
    const entries = batchIssues.map((issue) => {
        const quality = qualityResults.get(issue.number) ?? { score: 0, tier: "partial", breakdown: undefined };
        ...
        const recommendedAction = deriveBatchIssueAction(clusterIdx !== null, quality.score, visionAlignment);
        return {
...
export function deriveBatchIssueAction(isDuplicate, qualityScore, visionAlignment) {
    ...
    if (visionAlignment === "rejects")
        return "wontfix";
    if (visionAlignment === "error" || visionAlignment === "pending")
        return "flag";
...
function computeIssueStats(entries, clusters) {
