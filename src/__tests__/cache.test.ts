...
function makeEntry(number: number): CachedEntry {
  return {
    number,
    title: `PR #${number}`,
...
function makeCache(entries: CachedEntry[] = []): EmbeddingCache {
  return {
    version: 1,
...
describe("upsertEntry", () => {
  it("inserts a new entry", () => {
    const entries: CachedEntry[] = [makeEntry(1)];
  ...
  it("updates an existing entry by number", () => {
    const entries: CachedEntry[] = [makeEntry(1), makeEntry(2)];
