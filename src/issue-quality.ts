import type { Issue, IssueQualityBreakdown } from "./types.js";
...
function scoreDescription(body: string): number {
  const len = body.trim().length;
  if (len > 300) return 2.5;
  if (len > 150) return 1.5;
  if (len > 50) return 0.5;
  return 0.0;
}

function scoreReproSteps(body: string): number {
  let matches = 0;
  for (const pattern of REPRO_PATTERNS) {
    if (pattern.test(body)) matches++;
  }
  if (matches >= 3) return 2.5;
...
function scoreLabels(labels: string[]): number {
  if (labels.length >= 2) return 2.5;
  if (labels.length >= 1) return 1.5;
  return 0.0;
}

function scoreTemplate(body: string): number {
  let matches = 0;
...
export function scoreIssue(issue: Issue): {
  score: number;
...
export function scorePartialIssue(issue: {
  ...
  score: number;
  breakdown: Pick<IssueQualityBreakdown, "hasDescription" | "hasLabels">;
} {
