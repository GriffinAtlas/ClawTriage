import type { IssueBatchResult, BatchIssueTriageEntry, DuplicateCluster } from "./types.js";
...
function qualityIssues(entry: BatchIssueTriageEntry): string {
  const b = entry.qualityBreakdown;
  if (!b) return "\u2014";
  const issues: string[] = [];
  if (b.hasDescription < 1) issues.push("no description");
  if (b.hasReproSteps < 1) issues.push("no repro steps");
  if (b.hasLabels < 1) issues.push("no labels");
  if (b.followsTemplate < 1) issues.push("no template");
...
export function buildIssueSummaryIssue(result: IssueBatchResult): { title: string; body: string } {
  ...
  if (clusters.length > 0) {
    ...
    const entryMap = new Map<number, BatchIssueTriageEntry>();
    for (const e of entries) entryMap.set(e.issueNumber, e);
  ...
  for (const e of entries) {
    ...
    tableRows.push(
      `| #${e.issueNumber} | ${e.qualityScore} | ${formatLabels(e.labels)} | ${e.visionAlignment} | ${dupeLabel} | ${e.recommendedAction} | ${e.title} |`,
    );
  }
